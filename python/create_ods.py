...
data = xlrd.open_workbook('create_tab_enjoy.xlsx')
# 没有table描述
# 选择脚本类型
loop_flag = 1
emp_part_table = []
...
def add_tableinfo():
    global t_unique, t_cluster, t_collection, t_comment, t_part, t_database, t_filed, t_hive_external, t_hive_location, t_hive_location_local, t_hive_location_part, t_key, t_lines, t_map, t_part_comment, t_partition, t_stored
    ...
    if dialect_name == 'KHAN':
        for i in range(1, table_info.nrows):
            try:
                key = table_info.row_values(i)[1].strip().upper()
                #print(key)
                value = table_info.row_values(i)[3]
                t_part[key] = value
            except:
                pass
...
def getsql(key_name):
    ...
    if not table_sql.endswith(')'):
        ...
        for key, value in t_part.items():
            #print(key,value)
            if key == table_name.upper():
                if(value == 0): #0为分区，1为不分区
        #if table_name.upper() in t_part.keys():
            #if t_part[table_name.upper()] == 1:
                    table_sql = table_sql + '  etl_lad_dte DATE \n)'
                    before_time_nyr = '2017-05-03'
                    later_time_nyr = '2019-09-23'
...
with open('%s_goal_ENJOY.sql' % dialect_name.lower(), 'w') as file_goal:
